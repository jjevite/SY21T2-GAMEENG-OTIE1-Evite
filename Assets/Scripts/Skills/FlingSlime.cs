                ...
                if (hit.collider.tag == "Tile")
                {
                    ...
                    if (t.InteractSelectable)
                    {
                        ...
                        if (Physics.Raycast(hit.collider.gameObject.transform.position, Vector3.up, out hitUnitOnTop, 1))
                        {
                            ...
                            {
                                t.Target = true;
                                owner.GetComponent<Unit>().GameStats.Health.CurrentHP -= 10;
                                hitUnitOnTop.collider.GetComponent<Unit>().GameStats.Health.CurrentHP -= 25;
